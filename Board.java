...
class Board extends Canvas {
    ...
    private void drawTile(Graphics g, int col, int line, int color) {
        if (color == -1) {
            g.setColor(Color.lightGray);
            g.drawRect(col*TILE_SIZE, line*TILE_SIZE, TILE_SIZE, TILE_SIZE);
        } else {
    ...
    private void drawBlock(Block block, int pos, int col, int line, int color) {
        ...
        for (int t=0; t<4; t++) {
            ...
            tiles[line+tile.dy][col+tile.dx] = color;
        }
    }
    ...
    public void paint(Graphics g) {  
        ...
        for (int line = 0; line < lines; line++) {
            for (int col = 0; col < columns; col++) {
                ...
                if (tiles[line][col] != 0) {
                    drawTile(g, col+1, line+1, tiles[line][col]);
                }
            }
        }
    }
